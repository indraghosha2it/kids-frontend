...
export default function AdminEditToyProduct() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const productId = searchParams.get('id');
...
// Replace ALL THREE editor declarations with these:
...
const deliveryInfoEditor = useEditor({
  ...
  // Replace ALL existing editor sync useEffect hooks with these:
...
useEffect(() => {
  if (shortDescEditor && originalProduct?.shortDescription) {
    ...
    if (currentContent !== originalProduct.shortDescription) {
      ...
      console.log('Setting short description:', originalProduct.shortDescription);
    }
  }
...
useEffect(() => {
  if (fullDescEditor && originalProduct?.fullDescription) {
    ...
    if (currentContent !== originalProduct.fullDescription) {
      ...
      console.log('Setting full description:', originalProduct.fullDescription);
    }
  }
...
useEffect(() => {
  if (deliveryInfoEditor && originalProduct?.deliveryInfo) {
    ...
    if (currentContent !== originalProduct.deliveryInfo) {
      ...
      console.log('Setting delivery info:', originalProduct.deliveryInfo);
    }
  }
}, [deliveryInfoEditor, originalProduct?.deliveryInfo]);

  useEffect(() => {
  console.log('formData.shortDescription changed:', formData.shortDescription);
  console.log('formData.fullDescription changed:', formData.fullDescription);
  console.log('formData.deliveryInfo changed:', formData.deliveryInfo);
}, [formData.shortDescription, formData.fullDescription, formData.deliveryInfo]);
// ======================================
  // Add this useEffect right after your editor sync hooks
useEffect(() => {
...
// Replace your fetchProduct function with this complete version:
...
const fetchProduct = async () => {
  ...
  try {
    ...
    if (data.success) {
      ...
      console.log('Product fullDescription:', product.fullDescription);
      console.log('Product deliveryInfo:', product.deliveryInfo);
      setOriginalProduct(product);
      ...
      // IMPORTANT: Force editor updates by directly setting content
      // Use multiple timeouts to ensure editors are ready
    // In fetchProduct, replace the setTimeout with:
setTimeout(() => {
  ...
  const handleYoutubeUrlChange = (url) => {
    ...
    if (videoId) {
      ...
      // Clear any existing video upload
      if (videoUpload.preview?.startsWith('blob:')) {
        URL.revokeObjectURL(videoUpload.preview);
      }
  ...
  const removeVideo = () => {
    // If it's an uploaded video and not new, mark for deletion
 ...
 const hasChanges = () => {
  ...
  // Check images - only compare existing images (not new uploads)
